...
This document uses *quantile dotplots* to explore police presence in Chicago. 
The notebook uses the code from the following article. It changes the mean value
such that it is representative of the number of officers in an square mile on a 
given day. The text leaves in the original example for bus wait times until the 
last section, but all numbers have been updated. Please run all cells in order.  

Cite: Matthew Kay, Tara Kola, Jessica Hullman, Sean Munson. *When (ish) is My Bus? User-centered Visualizations of Uncertainty in Everyday, Mobile Predictive Systems*. CHI 2016. DOI: [10.1145/2858036.2858558](http://dx.doi.org/10.1145/2858036.2858558).
...
theme_set(theme_light() + theme(
    ...
    line=element_line(linewidth=.75)
))
```
...
```{r}
12877.95056159086/21.616805912410005
```

``` {r}
mu = log(21.616805912410005)
sigma = 0.2
x = seq(from=.01, to=50, length=10001)
d = dlnorm(x, mu, sigma)
density_df = data_frame(x, d)
density_df %>%
    ggplot(aes(x = x, y = d)) +
    geom_line() +
    geom_vline(xintercept = 21.6168, color="red") + 
    xlim(0, 50)
```
...
cdf_plot = ggplot(quantiles, aes(x=x, y=p_less_than_x)) +
    ...
    annotate("text", x = 50, y = 1, label = "cumulative distribution function", color="red", hjust=1, vjust=1.5) +
    xlim(c(-3.5, 50))
cdf_plot
...
```{r}
quantile_dotplot +
    geom_line(aes(y = d * 10.1), data = density_df, color="red")
```
...
Consequently, this purple line represents the number of officers I should expect in any Chicago police district on a given day to have about 18 officers across a square mile in a district throughout the day around 10% of the time.
